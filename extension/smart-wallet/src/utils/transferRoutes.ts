import { TokenExtension } from "./tokenUpdate";

export async function calculateTransferRoute(transferFromAddress: string, tokenTicker:string, value:bigint){
    const walletAddress = transferFromAddress;
    const tokenExtension = new TokenExtension(walletAddress);
    const balances = await tokenExtension.fetchBalances();
